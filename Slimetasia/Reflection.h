...
#define REFLECT_VIRTUAL(A)                                                                                             \
    ...
    {                                                                                                                  \
    public:                                                                                                            \
        A##Reflection() { Factory::registerType(#A, this); }                                                           \
        virtual std::vector<registration::variant> getProperties() override { return A::reflection.get_properties(); } \
        virtual std::vector<registration::class_> getParents() override { return A::reflection.get_parents(); }        \
        virtual bool hasVtable() override { return A::reflection.Vtable; }                                             \
        virtual registration::class_ getType() override { return A::reflection; }                                      \
    };                                                                                                                 \
...
#define REFLECT_TEMPLATE(A, B)                                                                                            \
    ...
    {                                                                                                                     \
    public:                                                                                                               \
        A##B##Reflection() { Factory::registerType(#A #B, this); }                                                        \
        virtual std::vector<registration::variant> getProperties() override { return A<B>::reflection.get_properties(); } \
        virtual std::vector<registration::class_> getParents() override { return A<B>::reflection.get_parents(); }        \
        virtual bool hasVtable() override { return A<B>::reflection.Vtable; }                                             \
        virtual registration::class_ getType() override { return A<B>::reflection; }                                      \
    };                                                                                                                    \
    class A##B##Factory : public ComponentFactory                                                                         \
    {                                                                                                                     \
    public:                                                                                                               \
        A##B##Factory() { Factory::registerType(#A #B, this); }                                                           \
        virtual void* create(GameObject* go) override { return reinterpret_cast<void*>(go->AddComponent<A<B>>()); }       \
        virtual void remove(GameObject* go) override { go->RemoveComponent<A<B>>(); }                                     \
        virtual void createIfDoesntExist(GameObject* go) override { go->AddIfDoesntExist<A<B>>(); }                       \
    };                                                                                                                    \
...
#define REFLECT_INIT(A)                                                                                                \
    ...
    {                                                                                                                  \
    public:                                                                                                            \
        A##Factory() { Factory::registerType(#A, this); }                                                              \
        virtual void* create(GameObject* go) override { return reinterpret_cast<void*>(go->AddComponent<A>()); }       \
        virtual void remove(GameObject* go) override { go->RemoveComponent<A>(); }                                     \
        virtual void createIfDoesntExist(GameObject* go) override { go->AddIfDoesntExist<A>(); }                       \
    };                                                                                                                 \
...
#define REFLECT_ENUM(A)                                                               \
    ...
    public:                                                                           \
        ...
        virtual std::vector<const char*> getEnums() override { return enums_; }       \
    };                                                                                \
...
#define RESOURCE_INIT(A)                                                                                               \
    ...
    {                                                                                                                  \
    public:                                                                                                            \
        A##Resource() { Factory::registerResource(#A, this); }                                                         \
        virtual std::vector<registration::variant> getProperties() override { return A::reflection.get_properties(); } \
        virtual std::vector<registration::class_> getParents() override { return A::reflection.get_parents(); }        \
        virtual bool hasVtable() override { return A::reflection.Vtable; }                                             \
        virtual registration::class_ getType() override { return A::reflection; }                                      \
...
namespace registration
{
    ...
    struct method
    {
        ...
        template <typename T, typename... Args> void invoke(T& obj, Args... a) {}
    };
    ...
    struct class_ : public Node
    {
        ...
        template <typename T> registration::property get_property(std::string n, T& obj)
        {
    ...
    template <typename T> auto GetType() { return T::reflection; }
