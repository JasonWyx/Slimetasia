...
namespace
{
    ...
    void glfwInstallCallbacks(GLFWwindow* window)
    {
    ...
    bool CreateFontsTexture()
    {
        ...
        io.Fonts->GetTexDataAsRGBA32(
            &pixels, &width, &height);  // Load as RGBA 32-bits (75% of the memory is wasted, but default font is so small) because it is more likely to be compatible with user's existing shaders. If
                                        // your ImTextureId represent a higher-level concept than just a GL texture id, consider calling GetTexDataAsAlpha8() instead to save on GPU memory.
...
namespace ImGuiUtils
{
    bool Init(GLFWwindow* window, bool install_callbacks, const char* glsl_version)
    {
        ...
        if (install_callbacks) glfwInstallCallbacks(window);
